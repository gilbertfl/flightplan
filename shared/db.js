...
function doSaveSegment(pool, awardId, position, row, resolve, reject) {
  try {
    pool.request()
        .input('awardId', sql.Int, awardId)
...
doSaveSegment[util.promisify.custom] = (pool, awardId, position, row) => {
  return new Promise((resolve, reject) => {
    doSaveSegment(pool, awardId, position, row, resolve, reject);
  });
};
async function saveSegment(pool, awardId, position, row) {
  var promisifiedSaveSegment = util.promisify(doSaveSegment);
  return await promisifiedSaveSegment(pool, awardId, position, row);
}

// TODO: do we need to do request in a transaction?
...
function doSaveAward(pool, requestId, row, resolve, reject) {
  try {
    const { segments } = row;
    delete row.segments;
...
doSaveAward[util.promisify.custom] = (pool, requestId, row) => {
  return new Promise((resolve, reject) => {
    doSaveAward(pool, requestId, row, resolve, reject);
  });
};
async function saveAward(pool, requestId, row) {
  var promisifiedSaveAward = util.promisify(doSaveAward);
  return await promisifiedSaveAward(pool, requestId, row);
}
...
async function saveAwards(pool, requestId, rows) {
  try {
    for (const row of rows) {
      var segments = await saveAward(pool, requestId, row);
      if (segments) {
        
        let segmentFns = [];
        for (let i=0; i<segments.length; i++) {
          segmentFns.push(saveSegment(pool, segments[i].awardId, i, segments[i].segment));
        }
    ...
    console.error("SaveAwards encountered exception, save failed.", e);
  }
}
